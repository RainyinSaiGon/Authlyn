...
>
> 1. Port the design system from `design/` faithfully — do not invent new tokens, components, or layout patterns that are not already in the design files.
...
Wire the production React frontend (`frontend/`) up to the Authlyn design system defined in `design/`. This means replacing the placeholder font stack and CSS tokens, porting all primitive UI components and layout shells to TypeScript, installing a router, and establishing a route skeleton that every subsequent screen task can fill in.

The design source of truth is `design/ds/colors_and_type.css` for tokens, `design/primitives.jsx` for components, and `design/screen.css` for utility classes and layout patterns.
...
- [ ] Copy all font files from `design/ds/fonts/` into `frontend/public/fonts/`
...
- [ ] Replace `frontend/src/styles/global.css`:
  ...
  - Merge all CSS custom properties from `design/ds/colors_and_type.css` — brand colors (`--brand-accent`, `--brand-mint`, `--brand-danger`, etc.), foreground/background scale, surface tokens, border, warning, spacing, radius, and shadow variables
  - Update `:root` `font-family` to `"Google Sans Code Proportional"` and `font-family` for mono contexts to `"Google Sans Code"`
- [ ] Add `frontend/src/styles/utilities.css` with the utility classes from `design/screen.css`: `.glass` variants, `.row`, `.col`, `.eye`, `.cap`, `.mono`, `.muted`, `.dim`, `.emph`, `.pill` tones, `.tbl`, `.codeblock`, `.progress`, `.divider`, `.kbd`
